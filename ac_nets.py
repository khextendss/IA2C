...
class CriticNetwork:
    def __init__(self, scope, num_state_features, num_actions, LR_C):
        ...
        with tf.variable_scope(self.scope + '/main_critic'):
            ...
            hidden_out = tf.nn.tanh(tf.layers.dense(self.state_input, 200, name="CH1")) 
            hidden_out = tf.layers.dense(hidden_out, 200, name="CH2") 
            self.Q = tf.layers.dense(hidden_out, num_actions, name="Q_val") 
            ...
            self.train_op = tf.train.AdamOptimizer(learning_rate=LR_C).minimize(self.loss)
            
        with tf.variable_scope(self.scope + '/target_critic'): #target network with delayed params (must have same arch as main)
            self.target_state_input = tf.placeholder(tf.float32, [None, num_state_features], name="state")
            hidden_out = tf.nn.tanh(tf.layers.dense(self.target_state_input, 200, name="CH1")) 
            hidden_out = tf.layers.dense(hidden_out, 200, name="CH2") 
            self.target_Q = tf.layers.dense(hidden_out, num_actions, name="Q_val")

        with tf.variable_scope(self.scope + '/configuration'):
            self.Q_value = tf.placeholder(tf.float32, [None], name="Q_value") 
            self.Q_cur = tf.placeholder(tf.float32, [None], name="Q_cur") 
            self.A_dist = tf.placeholder(tf.float32, [None], name="A_dist")
    ...
    def batch_update(self, x_vec, op_neuron_sel_vec, target_vec): #Updates only the main critic
        ...
        return self.sess.run(self.Q, feed_dict={self.state_input: x_vec})[0]
...
class ActorNetwork:
    def __init__(self, scope, num_state_features, num_actions, LR_A, BETA): 
        ...
        with tf.variable_scope(self.scope + '/actor'):
            self.state_input = tf.placeholder(tf.float32, [None, num_state_features])
            self.act_train = tf.placeholder(tf.uint8, [None])
            self.adv = tf.placeholder(tf.float32, [None])
            hidden_out = tf.nn.tanh(tf.layers.dense(self.state_input, 200, name="AH1"))
            hidden_out = tf.nn.relu(tf.layers.dense(hidden_out, 100, name="AH2")) 
            logits = tf.layers.dense(hidden_out, num_actions, name="AH3")
